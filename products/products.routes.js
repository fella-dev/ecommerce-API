...
const { createProduct, getAllProducts, getProductById, modifyprd, deleteProduct} = require("./product.controller");
...
router.delete("/:prdId",async(req,res)=>{
   ...
   res.send({message:"product deleted"})}catch{
    res.status(404)
    res.send({message:"product not found"})
   }
})
